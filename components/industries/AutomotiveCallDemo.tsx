...
import {
  automotiveCallLength,
  automotiveCallScript,
} from "@/lib/automotive";

const MS_PER_SECOND = 320;

function formatTimer(seconds: number) {
  const clamped = Math.min(seconds, automotiveCallLength);
  const mm = String(Math.floor(clamped / 60)).padStart(2, "0");
  const ss = String(clamped % 60).padStart(2, "0");
  return `${mm}:${ss}`;
}

export default function AutomotiveCallDemo() {
  ...
  const play = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed((prev) => {
        if (prev >= automotiveCallLength) {
          stop();
          return automotiveCallLength;
        }
        return prev + 1;
      });
    }, MS_PER_SECOND);
  ...
  useEffect(() => {
    ...
    if (reduced) {
      const id = requestAnimationFrame(() => setElapsed(automotiveCallLength));
  ...
  const done = elapsed >= automotiveCallLength;

  return (
    <div
      role="img"
      aria-label="Sample SvaraCall automotive service booking conversation"
    ...
    >
      ...
      <div className="mt-4 flex items-center justify-between border-b border-white/10 pb-4">
        <span className="tabular text-xs text-white/60">+91 98•• ••• 812</span>
      ...
      <ol className="mt-5 space-y-3">
        {automotiveCallScript.map((line, i) => {

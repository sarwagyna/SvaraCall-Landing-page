...
import { telecomCallLength, telecomCallScript } from "@/lib/telecom";

const MS_PER_SECOND = 320;

function formatTimer(seconds: number) {
  const clamped = Math.min(seconds, telecomCallLength);
  const mm = String(Math.floor(clamped / 60)).padStart(2, "0");
  const ss = String(clamped % 60).padStart(2, "0");
  return `${mm}:${ss}`;
}

export default function TelecomCallDemo() {
  ...
  const play = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed((prev) => {
        if (prev >= telecomCallLength) {
          stop();
          return telecomCallLength;
        }
        return prev + 1;
      });
    }, MS_PER_SECOND);
  ...
  useEffect(() => {
    ...
    if (reduced) {
      const id = requestAnimationFrame(() => setElapsed(telecomCallLength));
  ...
  const done = elapsed >= telecomCallLength;

  return (
    <div
      role="img"
      aria-label="Sample SvaraCall telecom bill reminder conversation"
    ...
    >
      ...
      <div className="mt-4 flex items-center justify-between border-b border-white/10 pb-4">
        <span className="tabular text-xs text-white/60">+91 98•• ••• 441</span>
          ...
          return (
            ...
            >
              ...
              >
                ...
                {"lang" in line && line.lang === "hi" ? (
                  <span lang="hi">{line.text}</span>
                ) : (
                  <span>{line.text}</span>
                )}
      ...
      >
        <span className="inline-flex items-center gap-2 rounded-pill bg-positive px-3 py-1.5 text-xs font-semibold text-white">
          ...
          Bill reminded · Pay link sent · Active
        </span>
        <button
          type="button"
          onClick={play}

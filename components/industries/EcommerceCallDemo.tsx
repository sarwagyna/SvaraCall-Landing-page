...
function formatTimer(seconds: number) {
  const clamped = Math.min(seconds, ecommerceCallLength);
  const mm = String(Math.floor(clamped / 60)).padStart(2, "0");
  const ss = String(clamped % 60).padStart(2, "0");
  return `${mm}:${ss}`;
}

export default function EcommerceCallDemo() {
  ...
  const play = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed((prev) => {
        if (prev >= ecommerceCallLength) {
          stop();
          return ecommerceCallLength;
        }
        return prev + 1;
      });
    }, MS_PER_SECOND);
  ...
  useEffect(() => {
    ...
    if (reduced) {
      const id = requestAnimationFrame(() => setElapsed(ecommerceCallLength));
  ...
  const done = elapsed >= ecommerceCallLength;

  return (
    <div
      role="img"
      aria-label="Sample SvaraCall COD verification conversation"
    ...
    >
      ...
      <div className="mt-4 flex items-center justify-between border-b border-white/10 pb-4">
        <span className="tabular text-xs text-white/60">+91 98•• ••• 642</span>
      ...
      <ol className="mt-5 space-y-3">
        {ecommerceCallScript.map((line, i) => {
      ...
      >
        <span className="inline-flex items-center gap-2 rounded-pill bg-positive px-3 py-1.5 text-xs font-semibold text-white">
          ...
          COD confirmed · Evening slot · Cleared to ship
        </span>
        <button
          type="button"
          onClick={play}

...
function formatTimer(seconds: number) {
  const clamped = Math.min(seconds, brokerageCallLength);
  const mm = String(Math.floor(clamped / 60)).padStart(2, "0");
  const ss = String(clamped % 60).padStart(2, "0");
  return `${mm}:${ss}`;
}

export default function BrokerageCallDemo() {
  ...
  const play = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed((prev) => {
        if (prev >= brokerageCallLength) {
          stop();
          return brokerageCallLength;
        }
        return prev + 1;
      });
    }, MS_PER_SECOND);
  ...
  useEffect(() => {
    ...
    if (reduced) {
      const id = requestAnimationFrame(() => setElapsed(brokerageCallLength));
  ...
  const done = elapsed >= brokerageCallLength;

  return (
    <div
      role="img"
      aria-label="Sample SvaraCall brokerage account activation conversation"
    ...
    >
      ...
      <div className="mt-4 flex items-center justify-between border-b border-white/10 pb-4">
        <span className="tabular text-xs text-white/60">+91 98•• ••• 704</span>
      ...
      <ol className="mt-5 space-y-3">
        {brokerageCallScript.map((line, i) => {

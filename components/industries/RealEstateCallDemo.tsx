...
import {
  realEstateCallLength,
  realEstateCallScript,
} from "@/lib/realEstate";

const MS_PER_SECOND = 320;

function formatTimer(seconds: number) {
  const clamped = Math.min(seconds, realEstateCallLength);
...
export default function RealEstateCallDemo() {
  ...
  const play = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed((prev) => {
        if (prev >= realEstateCallLength) {
          stop();
          return realEstateCallLength;
        }
        return prev + 1;
      });
    }, MS_PER_SECOND);
  ...
  useEffect(() => {
    ...
    if (reduced) {
      const id = requestAnimationFrame(() => setElapsed(realEstateCallLength));
  ...
  const done = elapsed >= realEstateCallLength;

  return (
    <div
      role="img"
      aria-label="Sample SvaraCall real estate portal lead conversation"
    ...
    >
      ...
      <ol className="mt-5 space-y-3">
        {realEstateCallScript.map((line, i) => {
      ...
      >
        <span className="inline-flex items-center gap-2 rounded-pill bg-positive px-3 py-1.5 text-xs font-semibold text-white">
          ...
          Qualified · Visit booked · Logged to CRM
        </span>
        <button
          type="button"
          onClick={play}

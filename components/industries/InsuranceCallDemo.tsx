...
function formatTimer(seconds: number) {
  const clamped = Math.min(seconds, insuranceCallLength);
...
export default function InsuranceCallDemo() {
  ...
  const play = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed((prev) => {
        if (prev >= insuranceCallLength) {
          stop();
          return insuranceCallLength;
        }
        return prev + 1;
      });
    }, MS_PER_SECOND);
  ...
  useEffect(() => {
    ...
    if (reduced) {
      const id = requestAnimationFrame(() => setElapsed(insuranceCallLength));
  ...
  const done = elapsed >= insuranceCallLength;

  return (
    <div
      role="img"
      aria-label="Sample SvaraCall insurance renewal conversation"
    ...
    >
      ...
      <div className="mt-4 flex items-center justify-between border-b border-white/10 pb-4">
        <span className="tabular text-xs text-white/60">+91 98•• ••• 318</span>
      ...
      <ol className="mt-5 space-y-3">
        {insuranceCallScript.map((line, i) => {
      ...
      >
        <span className="inline-flex items-center gap-2 rounded-pill bg-positive px-3 py-1.5 text-xs font-semibold text-white">
          ...
          Renewal intent · Link sent · Logged
        </span>
        <button
          type="button"
          onClick={play}

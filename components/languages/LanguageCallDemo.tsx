...
function formatTimer(seconds: number, max: number) {
...
type LanguageCallDemoProps = {
  page: LanguagePage;
  scriptClassName?: string;
};

export default function LanguageCallDemo({
  page,
  scriptClassName,
}: LanguageCallDemoProps) {
  ...
  const play = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed((prev) => {
        if (prev >= page.callLength) {
          stop();
          return page.callLength;
        }
        return prev + 1;
      });
    }, MS_PER_SECOND);
  }, [page.callLength, stop]);
  ...
  useEffect(() => {
    ...
    if (reduced) {
      const id = requestAnimationFrame(() => setElapsed(page.callLength));
  ...
  }, [play, stop, page.callLength]);

  const done = elapsed >= page.callLength;

  return (
    <div
      role="img"
      aria-label={`Sample SvaraCall ${page.name} conversation`}
      className="min-h-[480px] w-full rounded-card bg-ink p-6 text-white ring-1 ring-white/10"
    >
      <div className="flex items-center justify-between">
        ...
        <span className="tabular text-sm font-semibold text-primary">
          {formatTimer(elapsed, page.callLength)}
        </span>
      </div>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <span lang={page.langAttr} className={scriptClassName}>
                  {line.text}
                </span>
                {line.gloss ? (
